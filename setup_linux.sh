...
check_required_variables() {
  ...
  required_vars=(GITHUB_PROJECTS_DIR GITHUB_REPO_SETUP GITHUB_TOKEN GITHUB_USER_EMAIL GITHUB_USER_NAME)
...
focus_here() {
  ...
  if [ "${howManyRepos}" -gt 0 ]; then
    ...
    cd "${GITHUB_PROJECTS_DIR}" || {
      log_error "ERROR: ${GITHUB_PROJECTS_DIR} not found"
      exit 1
    }
...
setup_symbolic_linksX() {
  ...
  original_bin_dir="${GITHUB_PROJECTS_DIR}/bin"
...
#cp "$GITHUB_PROJECTS_DIR/$GITHUB_REPO_SETUP/.post_bashrc" "$HOME"
#cp "$GITHUB_PROJECTS_DIR/$GITHUB_REPO_SETUP/.bash_profile" "$HOME"
